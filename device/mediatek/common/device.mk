...
# Telephony
# kcm modify by liubaofu, 20220409, for custom project apns-conf
ifneq ($(wildcard $(MTK_TARGET_PROJECT_FOLDER)/apns-conf.xml),)
PRODUCT_COPY_FILES += $(MTK_TARGET_PROJECT_FOLDER)/apns-conf.xml:system/etc/apns-conf.xml:mtk
else
...
# kcm modify by liubaofu, 20220409, for custom project spn-conf
ifneq ($(wildcard $(MTK_TARGET_PROJECT_FOLDER)/spn-conf.xml),)
PRODUCT_COPY_FILES += $(MTK_TARGET_PROJECT_FOLDER)/spn-conf.xml:$(TARGET_COPY_OUT_SYSTEM_EXT)/etc/spn-conf.xml:mtk
else
...
ifeq ($(strip $(BUILD_AGO_GMS)), yes)
  ifeq ($(strip $(MSSI_CUSTOM_CONFIG_MAX_DRAM_SIZE)), 0x80000000)
    # kcm modify by liubaofu, 20220409, for customm gms	
    ifneq ($(wildcard $(MTK_TARGET_PROJECT_FOLDER)/gms_go_2gb.mk),)
    $(call inherit-product-if-exists, $(MTK_TARGET_PROJECT_FOLDER)/gms_go_2gb.mk)
    else
    ...
    # kcm modify by liubaofu, 20220409, for customm gms
    ifneq ($(wildcard $(MTK_TARGET_PROJECT_FOLDER)/gms_go.mk),)
    $(call inherit-product-if-exists, $(MTK_TARGET_PROJECT_FOLDER)/gms_go.mk)
    else
    # $(call inherit-product-if-exists, vendor/go-gms/products/gms.mk)
    $(call inherit-product-if-exists, vendor/google/gms.mk)
    endif
...
# kcm modify by liubaofu, 20220409, for customm gms
ifneq ($(wildcard $(MTK_TARGET_PROJECT_FOLDER)/gms.mk),)
$(call inherit-product-if-exists, $(MTK_TARGET_PROJECT_FOLDER)/gms.mk)
else
$(call inherit-product-if-exists, vendor/google/gms.mk)
endif
...
# KCM modified by liubaofu, 20220409 for custom project Ecclist
ifneq ($(wildcard $(MTK_TARGET_PROJECT_FOLDER)/EccList/EccList.mk),)
$(call inherit-product-if-exists, $(MTK_TARGET_PROJECT_FOLDER)/EccList/EccList.mk)
else
...
ifeq ($(strip $(MTK_IMS_SUPPORT)), yes)
    ifneq ($(strip $(MTK_BASIC_PACKAGE)), yes)
        ...
        ifeq ($(strip $(MTK_TELEPHONY_ADD_ON_POLICY)), 0)
            PRODUCT_PACKAGES += MtkGbaService
...
ifneq ($(wildcard vendor/mediatek/internal/mtklog_enable),)
  ...
  PRODUCT_PACKAGES += liblog_wifi_jni
    ...
    ifneq ($(strip $(MSSI_MTK_TC1_COMMON_SERVICE)), yes)
      ifneq ($(wildcard vendor/mediatek/proprietary/packages/apps/Contacts/Android.bp),)
        ...
        ifeq ($(strip $(MTK_PRODUCT_LINE)), tablet)
          DEVICE_PACKAGE_OVERLAYS += $(LOCAL_PATH)/overlay/local_account/tablet
        else ifeq ($(strip $(MTK_PRODUCT_LINE)), smart_phone)
          DEVICE_PACKAGE_OVERLAYS += $(LOCAL_PATH)/overlay/local_account/phone
        endif
        ifneq ($(strip $(BUILD_GMS)), yes)
...
# kcm added by liubaofu, 20220409
$(call inherit-product, device/along/common/device.mk)

# kcm added by liubaofu for prebuild app at 20220409
ifneq ($(wildcard vendor/prebuild_apps/products/$(MTK_TARGET_PROJECT).mk),)
$(call inherit-product-if-exists, vendor/prebuild_apps/products/$(MTK_TARGET_PROJECT).mk)
endif
# add end
